...
from image_models.timm.layers import PatchEmbed, Mlp, DropPath, ClassifierHead, to_2tuple, to_ntuple, trunc_normal_, \
...
class SwinTransformerBlock(nn.Module):
    ...
    def _calc_window_shift(self, target_window_size, target_shift_size) -> Tuple[Tuple[int, int], Tuple[int, int]]:
        te_input_resolution = to_2tuple(self.input_resolution)
        target_window_size = to_2tuple(target_window_size)
        target_shift_size = to_2tuple(target_shift_size)
        window_size = [r if r <= w else w for r, w in zip(te_input_resolution, target_window_size)]
        shift_size = [0 if r <= w else s for r, w, s in zip(te_input_resolution, window_size, target_shift_size)]
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, H, 1, W // 2, 2, C).permute(0, 1, 3, 4, 2, 5).flatten(3)
        x = self.norm(x)
        x = self.reduction(x)
        return x
